...
# Total experiments: 125 * 3 models = 375
...
def run_grid_search_for_year(year: int, features: int = 150, skip_scoring: bool = False):
    ...
    print(f"\n{'='*60}")
    print(f"Loading data for year {year} (will be reused by all {total_experiments} experiments)")
    print(f"{'='*60}")
    ...
    for model_name in GRID_SEARCH_CONFIG:
        print(f"\n{'='*60}")
        print(f"Starting {model_name.upper()} experiments ({model_counts[model_name]} combinations)")
        print(f"{'='*60}")
        ...
        for params in param_combinations:
            ...
            if experiment_num % 25 == 0:
                interim_file = RESULTS_DIR / f"grid_search_{year}_interim_{timestamp}.json"
    ...
    print(f"\n{'='*60}")
    print(f"All {total_experiments} experiments completed in {total_elapsed/60:.1f} minutes")
    print(f"{'='*60}")
    ...
    scores = [exp.get('score') for exp in experiment_results['experiments'] if exp.get('score')]
    if scores:
    ...
    # Summary by model
    for model_name in GRID_SEARCH_CONFIG:
        model_exps = [exp for exp in experiment_results['experiments'] if exp['model'] == model_name]
        ...
        if model_scores:
            ...
            best_exp = max([exp for exp in model_exps if exp.get('score')], key=lambda x: x['score'])
    ...
    with open(csv_file, 'w', encoding='utf-8') as f:
        # Header
        f.write("experiment_num,model,short_name,score,training_time_sec,status,params\n")
        for exp in sorted(experiment_results['experiments'], key=lambda x: x.get('score') or 0, reverse=True):
            score = exp.get('score', '')
            f.write(f"{exp['experiment_num']},{exp['model']},{exp['short_name']},{score},{exp.get('training_time_sec', '')},{exp['status']},\"{exp['params']}\"\n")
...
def main():
    parser = argparse.ArgumentParser(description='Run grid search hyperparameter tuning experiment')
    parser.add_argument('--year', type=int, nargs='+', required=True,
                        help='Year(s) to run experiment for (e.g., 2020 or 2020 2021 2022)')
    parser.add_argument('--features', type=int, default=150,
    ...
    print(f"Estimated time: {total_experiments * len(args.year) * 2 / 60:.0f} - {total_experiments * len(args.year) * 5 / 60:.0f} minutes")
    print(f"{'='*70}")
    ...
    if len(args.year) > 1:
        combined_file = RESULTS_DIR / f"grid_search_combined_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        for exp in all_experiments:
            ...
            if key not in config_scores:
                config_scores[key] = {'scores': [], 'params': exp['params'], 'model': exp['model']}
            if exp.get('score'):
        ...
        for i, cfg in enumerate(config_averages[:10], 1):
            print(f"{i:<5} {cfg['model']:<15} {cfg['short_name']:<30} {cfg['average_score']:.5f}      {cfg['years_count']}")
